...
module.exports = {
  ...
  bootstrap({ strapi }) {
    ...
    io.on(SOCKET_EVENTS.CONNECTION, (socket) => {
      ...
      socket.on(SOCKET_EVENTS.DISCONNECT, handleDisconnect(socket, activeUsers, io));
    });
  }
};

// Handling for user joining
const handleJoin = (socket, activeUsers, io) => ({ username }) => {
...
// Handling for sending messages
const handleSendMessage = (strapi) => async ({ username, text }) => {
  try {
